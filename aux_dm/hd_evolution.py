...
def calculate_rk3_kval(h, ku, ka, kphi1, kphi2, kz, kOmega, kA_r, alpha, a, u, F1, F2, phi1, phi2, z, Omega, A_r, rhos):
	...
	for i in range(NUM_VPOINTS, NUM_SPOINTS-2):
		...
		if i == NUM_VPOINTS:
			...
			ku[i,:] 		= h * du_dt(i, rhos[i], u[i,:], phi1_, phi2_, z_, a_, a_p1h, a_m1h, \
				alpha_, alpha_p1h, alpha_m1h, F1[i,:], F1[i-1,:], F2[i,:], F2[i-1,:])
			ka[i] 			= h * fa(i, alpha_, a_, u[i,:], phi1_, phi2_)
...
def evolution_charge(cons,prim,sc1,sc2,A_r,z,Omega,a,alpha,curr,next):
	...
	k1a[i] 			= h * fa(i, (alpha[curr,i]+alpha[curr,i-1])/2, a[curr,i], u[i,:], phi1[i,:], phi2[i,:])
	...
	k1a[i] 			= h * fa(i, 1/a[curr,i], a[curr,i], u[i,:], phi1[i,:], phi2[i,:])
	...
	k2a[i] 			= h * fa(i, (alpha[curr,i]+alpha[curr,i-1])/2, ak1[i], uk1[i,:], phi1k1[i,:], phi2k1[i,:])
	...
	k2a[i] 			= h * fa(i, 1/ak1[i], ak1[i], uk1[i,:], phi1k1[i,:], phi2k1[i,:])
	...
	i = NUM_SPOINTS-2
	r_p1h = R(i+1/2)
	k3a[i] 			= h * fa(i, (alpha[curr,i]+alpha[curr,i-1])/2, ak12[i], uk12[i,:], phi1k12[i,:], phi2k12[i,:])
	...
	k3a[i] 			= h * fa(i, 1/ak12[i], ak12[i], uk12[i,:], phi1k12[i,:], phi2k12[i,:])
	...
	for i in range(NUM_SPOINTS-1, NUM_VPOINTS, -1): # start at last boundary and move inwards
		...
		alpha[next,i-1] = falpha(alpha[next,i], R(i), cons[next,i,:], prim[next,i,:], phi1, phi2, z_, a[next,i])
